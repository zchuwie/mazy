...
export const getConfig = () => {
  const gameConfig = sessionStorage.getItem("gameConfig")
    ? JSON.parse(sessionStorage.getItem("gameConfig"))
    : null;
  return gameConfig;
};
...
export const renderArenaConfig = (p, config, map, mode, bullet) => {
  ...
  const getTankCharacter = (tankName) => {
    return (
      tankCharacters.find((char) => char.name === tankName) || tankCharacters[0]
    );
  };

  const player1Image = getTankImage(config?.player1);
  const player2Image = getTankImage(config?.player2);
  const player1Character = getTankCharacter(config?.player1);
  const player2Character = getTankCharacter(config?.player2);
