...
const sketch = (p) => {
  ...
  function renderTutorial() {
    ...
    tutorialSlides.forEach((s, i) =>
      s.classList.toggle("active", i === tutorialStepIndex),
    );
    tutorialDots.forEach((d, i) =>
      d.classList.toggle("active", i === tutorialStepIndex),
    );
    ...
    if (currentStepEl)
      currentStepEl.textContent = String(tutorialStepIndex + 1);
  ...
  p.preload = function () {
    ...
    characterSelectSound = p.loadSound(
      "../assets/audio/sfx/character-select.wav",
    );
    characterSelectSound.setVolume(0.6);

    characterConfirmSound = p.loadSound(
      "../assets/audio/sfx/character-selected.wav",
    );
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // =====================
    tutorialSlides = Array.from(
      document.querySelectorAll("#tutorialSlides .slide"),
    );
    ...
    closeOptionsBottom?.addEventListener("click", () => {
      playClick();
      closeModal(optionsModal);
    });
    ...
    closeCreditsBottom?.addEventListener("click", () => {
      playClick();
      closeModal(creditsModal);
    });
    ...
    creditsModal?.addEventListener("click", (e) => {
      if (e.target === creditsModal) closeModal(creditsModal);
    });
    ...
    resetOptions?.addEventListener("click", () => {
      ...
      if (characterSelectSound && characterSelectSound.isLoaded())
        characterSelectSound.setVolume(sfxVol);
      if (characterConfirmSound && characterConfirmSound.isLoaded())
        characterConfirmSound.setVolume(sfxVol);
  ...
  function startGame() {
    ...
    const gameConfig = {
      ...
      player2: playerCount === 2 ? characters[player2Index].name : null,
      time: 180,
    };
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (!player1Selected) {
      if (e.key === "a" || e.key === "A") {
        player1Index =
          (player1Index - 1 + characters.length) % characters.length;
        renderCharacters();
    ...
    if (!player2Selected && selectedMode !== "bot") {
      if (e.key === "ArrowLeft") {
        player2Index =
          (player2Index - 1 + characters.length) % characters.length;
        renderCharacters();
        selectCharacterSound();
      }
